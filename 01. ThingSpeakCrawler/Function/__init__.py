...
def main(mytimer: func.TimerRequest) -> None:
    ...
    url: str = "https://tgdatasanitization.azurewebsites.net/api/Function"

    c: Crawler = Crawler(url_ts.format(ts_filter_date.strftime("%Y-%m-%d")))    
    w: Weather = c.GetWeatherFromService()

    with open('data.txt', 'w') as outfile:
        json.dump(w.to_dict()["feeds"], outfile)

    requests.request("POST", url, data=json.dumps(w.to_dict()["feeds"]), headers=headers)
